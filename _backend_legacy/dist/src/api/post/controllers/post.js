...
const hasOwnAuthorFilter = (filters, userId) => {
    var _a, _b, _c, _d;
    if (!filters || !userId)
        return false;
    ...
    const authorEq = (_a = authorFilter.$eq) !== null && _a !== void 0 ? _a : (_b = authorFilter.$in) === null || _b === void 0 ? void 0 : _b[0];
    ...
    const eqValue = (_c = idFilter.$eq) !== null && _c !== void 0 ? _c : (_d = idFilter.$in) === null || _d === void 0 ? void 0 : _d[0];
...
exports.default = strapi_1.factories.createCoreController('api::post.post', ({ strapi }) => ({
    async create(ctx) {
        ...
        const rawCardId = (_g = (_f = (_e = (_d = payload.cardId) !== null && _d !== void 0 ? _d : payload.card_id) !== null && _e !== void 0 ? _e : payload.attached_card) !== null && _f !== void 0 ? _f : payload.attachedCard) !== null && _g !== void 0 ? _g : payload.card;
    ...
    async find(ctx) {
        ...
        const filters = ((_b = (_a = ctx.query) === null || _a === void 0 ? void 0 : _a.filters) !== null && _b !== void 0 ? _b : {});
        const { sort, pagination, populate } = ctx.query;
        // Check if we are filtering by author (specifically the current user)
        const isAuthorFilter = hasOwnAuthorFilter(filters, user === null || user === void 0 ? void 0 : user.id);
        let queryFilters = { ...filters };
        let queryStatus = 'published';
        if (isAuthorFilter && (user === null || user === void 0 ? void 0 : user.id)) {
            ...
            const currentPublicFilter = typeof filters.is_public === 'object' && filters.is_public !== null
                ? filters.is_public
                : undefined;
            queryFilters.is_public = currentPublicFilter !== null && currentPublicFilter !== void 0 ? currentPublicFilter : { $eq: true };
        ...
        try {
            ...
            const safeResults = results.map((post) => {
                ...
                let userVote = null;
                if (user) {
                    const upvoters = (post.upvoted_by || []).map((u) => u.id);
                    const downvoters = (post.downvoted_by || []).map((u) => u.id);
    ...
    async vote(ctx) {
        ...
        strapi.log.info(`Vote: id=${id}, type=${type}, userId=${user === null || user === void 0 ? void 0 : user.id}`);
        try {
            ...
            const postAny = post;
            // Map to Document IDs
            const upvoters = (postAny.upvoted_by || []).map((u) => u.documentId).filter(Boolean);
            const downvoters = (postAny.downvoted_by || []).map((u) => u.documentId).filter(Boolean);
            ...
            if (newUpvoters.includes(userDocId))
                userVoteStatus = 'up';
            if (newDownvoters.includes(userDocId))
                userVoteStatus = 'down';
