  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        message,
        type,
        pageContext: context || window.location.pathname,
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4 sm:px-6">
      <div className="relative w-full max-w-md rounded-3xl border-2 border-[#d2a06f] bg-[#fff9eb] text-[#3c2415] shadow-[0_8px_0_#c99063,0_18px_30px_rgba(0,0,0,0.2)] p-6 sm:p-8">
        ...
        <h2 className="text-2xl font-extrabold text-[#d26d75] mb-2">
          Обратная связь
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <p className="block text-sm font-semibold text-[#5e4632] mb-1">
              Тип сообщения
            </p>
          ...
          <div>
            <label
              ...
              className="block text-sm font-semibold text-[#5e4632] mb-1"
            >
              Сообщение
            </label>
            <textarea
